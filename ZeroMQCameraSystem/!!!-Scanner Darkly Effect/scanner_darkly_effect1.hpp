...
private:
	...
	cv::Mat extractEdges(const cv::Mat& image) {
		cv::Mat gray, blur, edges, dilated;
	...
	cv::Mat combineEffect(const cv::Mat& quantized, const cv::Mat& edges) {
		...
		cv::Mat mask_inv = ~edges;
		cv::Mat mask_inv_bgr;
		cv::cvtColor(mask_inv, mask_inv_bgr, cv::COLOR_GRAY2BGR);

		cv::bitwise_and(quantized, mask_inv_bgr, result);
